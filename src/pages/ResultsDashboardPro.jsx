...
import { Award, Users, Target, BarChart3, RefreshCw, Search, Trophy, TrendingUp, Calendar, Star } from 'lucide-react';
...
// Gradient stat card matching Home.js style
...
const fetchAllData = async () => {
  ...
  const allResults = useMemo(() => {
    const r = [];
    scorings.forEach(s => { const ev = s.eventId || {}; (s.participants || []).forEach(p => r.push({ ...p, eventName: ev.eventName || 'Unknown', section: ev.section || s.section || '', eventType: ev.eventType || '' })); });
    return r;
  }, [scorings]);
  ...
  const buildStandings = (results) => {
    ...
    results.forEach(r => {
      ...
      const m = map[p]; m.totalPoints += r.totalPoints || 0;
      if (r.position === '1') m.firsts++; if (r.position === '2') m.seconds++; if (r.position === '3') m.thirds++;
      if (r.grade === 'A') m.gradeA++; if (r.grade === 'B') m.gradeB++; if (r.grade === 'C') m.gradeC++;
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    ...
    return (<Box sx={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 2, p: 1.5, boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
  ...
  const FilterBar = ({ showSearch = false, showDivision = true }) => (
    <Box display="flex" gap={1.5} flexWrap="wrap" alignItems="center" sx={{ mb: 2.5 }}>
      ...
      {showDivision && (<FormControl size="small" sx={{ minWidth: 160 }}><InputLabel>Division</InputLabel><Select value={selectedDivision} label="Division" onChange={e => setSelectedDivision(e.target.value)}><MenuItem value="">All Divisions</MenuItem>{DIVISION_CONFIG.map(d => <MenuItem key={d.key} value={d.key}>{d.label}</MenuItem>)}</Select></FormControl>)}
      ...
      <Box sx={{ minHeight: '100vh', background: 'linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%)' }}>
        <Container maxWidth="xl" sx={{ py: 3 }}>
          ...
          <Box className="no-print" sx={{ mb: 3 }}>
            {/* Welcome banner like Home.js */}
            <Paper sx={{
              p: 3, mb: 2.5, borderRadius: 3, position: 'relative', overflow: 'hidden',
              background: 'linear-gradient(135deg, #1E40AF 0%, #3B82F6 50%, #6366F1 100%)', color: '#fff'
            }}>
              <Box sx={{ position: 'absolute', right: 20, top: '50%', transform: 'translateY(-50%)', opacity: 0.08 }}>
                <Trophy size={120} />
              </Box>
            ...
            <Box className="print-area">
              ...
              {activeView === 'overview' && (<Box>
                ...
                <Paper sx={{ p: 2.5, borderRadius: 3, mb: 2.5, border: '1px solid #E2E8F0' }}>
                  <Box display="flex" alignItems="center" gap={1} mb={2}><Trophy size={18} color="#D97706" /><Typography sx={{ fontWeight: 800, fontSize: '16px' }}>Division Champions</Typography></Box>
                  <Grid container spacing={2}>
                    {DIVISION_CONFIG.map(d => {
                      const top = divisionStandings[d.key]?.[0]; const count = divisionStandings[d.key]?.length || 0;
                      return (
                        <Grid item xs={12} md={4} key={d.key}>
                          <Box sx={{ p: 2, borderRadius: 2, background: `${d.color}06`, border: `1px solid ${d.color}20` }}>
                            <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
                              <Box><Typography sx={{ fontWeight: 700, fontSize: '15px', color: d.color }}>{d.label}</Typography><Typography sx={{ fontSize: '11px', color: '#94A3B8' }}>{d.desc} · {count} parishes</Typography></Box>
                            ...
                            </Box>) : <Typography sx={{ color: '#CBD5E1', fontSize: '13px' }}>No results yet</Typography>}
                          </Box>
                        </Grid>
                      );
                    })}
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} md={7}>
                    <Paper sx={{ p: 2.5, borderRadius: 3, border: '1px solid #E2E8F0' }}>
                      <Typography sx={{ fontWeight: 700, fontSize: '15px', mb: 2 }}>Medal Tally</Typography>
                      <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={medalsChartData} layout="vertical" margin={{ left: 5, right: 20 }}>
                          <CartesianGrid strokeDasharray="3 3" stroke="#F1F5F9" /><XAxis type="number" tick={{ fill: '#64748B', fontSize: 11 }} /><YAxis dataKey="name" type="category" tick={{ fill: '#334155', fontSize: 11 }} width={100} />
                          <Tooltip content={<CustomTooltip />} /><Legend /><Bar dataKey="Gold" fill="#EAB308" radius={[0, 4, 4, 0]} /><Bar dataKey="Silver" fill="#94A3B8" radius={[0, 4, 4, 0]} /><Bar dataKey="Bronze" fill="#B45309" radius={[0, 4, 4, 0]} />
                        </BarChart>
                      </ResponsiveContainer>
                    </Paper>
                  </Grid>
              ...
              {/* ===== STANDINGS ===== */}
              {activeView === 'standings' && (<Box><FilterBar />
              ...
              {/* ===== DIVISIONS ===== */}
              {activeView === 'divisions' && (<Box><FilterBar showDivision={false} />{DIVISION_CONFIG.map(d => <DivisionBlock key={d.key} divKey={d.key} standings={divisionStandings[d.key] || []} />)}</Box>)}

              {/* ===== EVENTS ===== */}
              {activeView === 'events' && (<Box><FilterBar showSearch />
                <Paper sx={{ borderRadius: 3, border: '1px solid #E2E8F0', overflow: 'hidden' }}>
              ...
              {/* {activeView === 'topscorers' && (<Box><FilterBar showSearch />
                <Paper sx={{ borderRadius: 3, border: '1px solid #E2E8F0', overflow: 'hidden' }}>
                  ...
                  <TableContainer><Table size="small" sx={tableStyles}><TableHead><TableRow>
                    ...
                    {topScorers.map((p, idx) => (<TableRow key={idx} sx={{ background: idx < 3 ? '#FAFBFE' : 'transparent', '&:hover': { background: '#F8FAFC' } }}>
                      <TableCell><Typography sx={{ fontWeight: 700, fontSize: '13px', color: idx < 3 ? '#2563EB' : '#94A3B8' }}>{idx + 1} {idx < 3 && ['🥇', '🥈', '🥉'][idx]}</Typography></TableCell>
                      <TableCell sx={{ fontWeight: 600 }}>{p.participantName}</TableCell><TableCell>{p.parish}</TableCell><TableCell>{p.eventName}</TableCell>
