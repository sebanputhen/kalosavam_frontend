...
const ResultsDashboardPro = () => {
  ...
  // FIX: key includes section so same-named events in different sections don't merge
  const stageWinnersMap = useMemo(() => {
    if (activeView !== 'stages') return {};
    const eventStageMap = {};
    events.forEach(e => { eventStageMap[`${e.eventName}|${e.section}`] = getEventStage(e); });
    const map = {};
    allResults.forEach(r => {
      const stage = eventStageMap[`${r.eventName}|${r.section}`] || 'Unknown';
  ...
  // FIX: venue winnersLookup keyed by eventName|section
  const venueData = useMemo(() => {
    if (activeView !== 'venues' || !stageAllocation?.venues) return [];
    const winnersLookup = {};
    allResults.forEach(r => {
      if (r.position && ['1', '2', '3'].includes(r.position)) {
        const wKey = `${r.eventName}|${r.section}`;
        if (!winnersLookup[wKey]) winnersLookup[wKey] = [];
        winnersLookup[wKey].push({ position: r.position, name: r.participantType === 'Group' ? r.parish : r.participantName, parish: r.parish, grade: r.grade, totalPoints: r.totalPoints });
      }
    });
    return stageAllocation.venues.filter(v => v.venueId).map(v => {
      const venue = v.venueId;
      const venueEvents = (v.eventIds || []).filter(Boolean);
      const scoredCount = venueEvents.filter(e => winnersLookup[`${e.eventName}|${e.section}`]?.length > 0).length;
          ...
          <Box sx={{
            ...
            <Box display="flex" gap={isSmall ? 2 : 3} mt={2} flexWrap="wrap">
              {[
                ...
                { val: d.firsts + d.seconds + d.thirds, label: 'Prize' },
              ...
              {activeView === 'overview' && (
                <Box>
                  ...
                  {/* <Grid container spacing={isMobile ? 1.5 : 2.5} sx={{ mb: isMobile ? 2 : 3 }}>
                    ...
                    <Grid item xs={6} md={3}><StatCard label="Prizes" value={totalMedals} sub="🥇🥈🥉" color="#D97706" icon={<Award size={22} />} /></Grid>
                  </Grid> */}
                  ...
                  {isMobile ? (
                    ...
                    <GlassCard sx={{ overflow: 'hidden' }}>
                      ...
                      <TableContainer><Table size="small" sx={tableStyles}><TableHead><TableRow>
                        <TableCell width={50}>#</TableCell><TableCell>Parish</TableCell>
                        ...
                        <TableCell align="center">A</TableCell><TableCell align="center">B</TableCell><TableCell align="center">C</TableCell>
                ...
                <Box>
                  ...
                  <Grid container spacing={isMobile ? 1.5 : 2.5} sx={{ mb: isMobile ? 2 : 3 }}>
                    {availableStages.map((stage) => {
                      ...
                      const scoredCount = stageEvents.filter(e => stageWinnersMap[`${stage}|${e.eventName}|${e.section}`]?.length > 0).length;
                  ...
                  {Object.entries(stageGroupedEvents).filter(([stage]) => !selectedStage || stage === selectedStage).map(([stage, stageEvents]) => {
                    ...
                    const scoredCount = filtered.filter(e => stageWinnersMap[`${stage}|${e.eventName}|${e.section}`]?.length > 0).length;
                    const stageColor = stage === 'On Stage' ? '#2563EB' : '#10B981';
                    return (
                        ...
                        {isMobile ? (
                          <Box>
                            {filtered.map((event, idx) => (
                              <EventCard key={event._id} event={event} winners={stageWinnersMap[`${stage}|${event.eventName}|${event.section}`]} onParishClick={openParishModal} />
                            ))}
                          </Box>
                        ) : (
                          <TableContainer><Table size="small" sx={tableStyles}><TableHead><TableRow>
                            ...
                            {filtered.map((event, idx) => {
                              const winners = (stageWinnersMap[`${stage}|${event.eventName}|${event.section}`] || []).sort((a, b) => +a.position - +b.position);
                              const w = pos => winners.find(x => x.position === pos);
                              const isScored = winners.length > 0;
                              return (
                    ...
                    {venueData.map((v, vi) => {
                      ...
                      return (
                        <GlassCard key={v.venueId} sx={{ overflow: 'hidden', mb: isMobile ? 2 : 3 }}>
                          ...
                          {isMobile ? (
                            <Box>{v.events.map((event, idx) => <EventCard key={event._id} event={event} winners={v.winnersLookup[`${event.eventName}|${event.section}`]} onParishClick={openParishModal} />)}</Box>
                          ) : (
                            <TableContainer><Table size="small" sx={tableStyles}><TableHead><TableRow>
                              <TableCell>Sl</TableCell><TableCell>Event</TableCell><TableCell>Section</TableCell><TableCell>Type</TableCell><TableCell>Gender</TableCell>
                              <TableCell align="center">🥇 First</TableCell><TableCell align="center">🥈 Second</TableCell><TableCell align="center">🥉 Third</TableCell>
